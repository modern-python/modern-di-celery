...
The living truth about what `modern-di-celery` does **now** — one file per
...
- [`dependency-injection.md`](dependency-injection.md) — wiring a `modern-di`
  container into a Celery app: `setup_di` + worker-signal lifecycle, the
  per-task container seam, `FromDI`/`inject` resolution, and the `DITask`
  base class.
